...
def get_scatter_plot(entered_site, payload):
    ...
    if entered_site == 'ALL' or entered_site is None:
       #filtered_df = filtered_range_df.groupby(['Booster Version Category','Payload Mass (kg)'])['class'].mean().reset_index()
        filtered_df = filtered_range_df.groupby(['Booster Version Category','Payload Mass (kg)']).agg({
            'class': 'mean'
        }).reset_index()
        ...
        if filtered_df.empty:
            return px.scatter(title='No Data Available')  # Handle empty data
        else:
            #filtered_df_group = filtered_range_df.groupby(['Booster Version Category','Payload Mass (kg)'])['class'].mean().reset_index()
            filtered_df_group = filtered_df.groupby(['Booster Version Category','Payload Mass (kg)']).agg({
            'class': 'mean'
            }).reset_index()
...
# 2.5-5k
# Which F9 Booster Version Category (v1.0, v1.1, FT, B4, B5, etc.) has the highest launch success rate?
# F10